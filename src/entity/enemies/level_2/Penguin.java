...
public class Penguin extends Enemy {
	...
	private final int[] numFrames = { 1, 8, 8 };
	...
	public Penguin(TileMap tileMap, Player player) {
		super(tileMap);
		this.player = player;
		...
		try {
			...
			BufferedImage spritesheet = ImageIO
					.read(getClass().getResourceAsStream("/sprites/enemies/level_2/Penguin.gif"));
			...
			for (int i = 0; i < numFrames.length; i++) {
				...
				for (int j = 0; j < numFrames[i]; j++) {
					if (i == 0) {
						...
						movement[j] = spritesheet.getSubimage(j * width, (i - 1) * height, width, height);
					}
				}
	...
	private void getNextPosition() {
		...
		if (inMoveRange && !attacking && player.getX() != this.getX()) {
			isTrigger = true;
